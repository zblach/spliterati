  ...
  class CryptoError extends SignedError {}
    ...
    export class Shard {
      ...
      static unpack(bytes: Uint8Array | null): (Shard | null) {
        ...
        return new Shard(
          <Uint8Array>bytes.slice(...slicer.nextRange(KEYID_LENGTH)),
    ...
    export function generate(t: uint8, n: uint8, opts: {message?:Uint8Array, keyID?:Uint8Array} = {}): {
      ...
      if (opts.keyID != null) {
        if (opts.keyID.length !== KEYID_LENGTH) {
          throw new SyntaxError(`keyID must be ${KEYID_LENGTH} characters long`);
        }
        keyID = opts.keyID;
      } else {
        keyID = nacl.randomBytes(KEYID_LENGTH);
      }
    ...
    export function reconstruct(publicKey: Uint8Array, shards: Uint8Array[]): Uint8Array {
      ...
      if (!Uint8ArrayEqual(keyPair.publicKey, publicKey)) {
        throw new CryptoError('different publicKey than expected');
      }
      const sampleMessage = randomBytes(32);
      const signed = nacl.sign(sampleMessage, keyPair.secretKey);
      if (!Uint8ArrayEqual(nacl.sign.open(signed, publicKey)!, sampleMessage)) {
        throw new CryptoError('could not reconstruct expected private key');
      }
