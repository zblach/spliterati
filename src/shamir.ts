...
// @ts-ignore
import type { uint8 } from './uint8.ts';

class Polynomial {
  static readonly field = 256; // GF(2^8)

  private static readonly logTable: uint8[] = [
  ...
  private static readonly expTable: uint8[] = [
  ...
  private static randByte(): uint8 {
    return <uint8>Math.floor(Math.random() * this.field);
  }

  // These functions operate over a galois field of 2^8
  private static add(a: uint8, b: uint8): uint8 {
    // eslint-disable-next-line no-bitwise
    return <uint8>(a ^ b);
  }

  private static mul(a: uint8, b: uint8): uint8 {
    const sum = (this.logTable[a] + this.logTable[b]) % (this.field - 1);
    return this.expTable[sum];
  }

  private static div(a: uint8, b: uint8): uint8 {
    if (b === 0) {
      throw new RangeError('div zero');
    }

    const fm1 = this.field - 1;
    const diff = ((this.logTable[a] - this.logTable[b]) + fm1) % fm1;

    return this.expTable[diff];
  }

  private readonly coefficients: Uint8Array
  // index is the x exponent.
  ...
  constructor(intercept: uint8, degree: uint8) {
    ...
    for (let i = 1; i < this.coefficients.length; i++) {
      this.coefficients[i] = Polynomial.randByte();
    }
  }
  ...
  evaluate(x: uint8): uint8 {
    ...
    for (let i = degree - 1; i >= 0; i--) {
      out = Polynomial.add(Polynomial.mul(out, x), <uint8> this.coefficients[i]);
    }
  ...
  static interpolate(xs: Uint8Array, ys: Uint8Array, x: uint8): uint8 {
    ...
    for (let i = 0; i < limit; i++) {
      ...
      for (let j = 0; j < limit; j++) {
        ...
        basis = this.mul(basis, this.div(
          this.add(x, <uint8>xs[j]),
          this.add(<uint8>xs[i], <uint8>xs[j]),
        ));
      }
      result = this.add(result, this.mul(<uint8>ys[i], basis));
    }
...
export module Shamir {
  ...
  export function split(data: Uint8Array, n: uint8, t: uint8): Uint8Array[] {
    ...
    if (n >= 255) {
      throw new SyntaxError('too many shares for this galois field');
    }
    ...
    data.forEach((val, idx) => {
      const p = new Polynomial(<uint8>val, <uint8>(t - 1));
      xs.forEach((x, i) => {
   ...
   *
   * NOTE: if less than ${t} parts are provided, nonsense value will be returned.
   * NOTE: if shares are provided from different splits, nonsense value will be returned.
   *
   * @param {Uint8Array[]} parts - ${t}..${n} parts of a split secret
   *
   * @return {Uint8Array} - the reassembled secret (see NOTEs)
   */
  export function combine(parts: Uint8Array[]): Uint8Array {
    if (parts.length < 2) {
      throw new SyntaxError('need 2 or more parts');
    }

    const expectedLen = parts[0].length;
    ...
    for (let c = 0; c < secret.length; c++) {
      ...
      secret[c] = Polynomial.interpolate(xSamples, ySamples, 0);
    }
